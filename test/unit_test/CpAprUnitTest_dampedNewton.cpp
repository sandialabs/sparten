...
#include <unit_test/CpAprUnitTest_dampedNewton.h>
...
void CpAprUnitTest_dampedNewton::SetUp()
{
  ...
  _cpApr= new unitTest::CpAprDampedNewtonUnitTestOnly<CpAprSparseValue, CpAprKruskalValue, CpAprElemIdx, CpAprSubIdx>(_tolerance, _maxOuterIter, _maxInnerIter, _offset, _offsetTolerance, _progressInterval, _eps);
...
void CpAprUnitTest_dampedNewton::TearDown()
{
  if (_cpApr != nullptr)
  {
    delete _cpApr;
...
TEST_F(CpAprUnitTest_dampedNewton, compute)
{
  // KruskalTensor
...
TEST_F(CpAprUnitTest_dampedNewton, protected_methods)
{
  // KruskalTensor
  auto *kTensor = new sparten::KruskalTensor<double, sparten::type::SubIdx>(_nDimSmall, _nComponentSmall, _sizeDimSmall);
  sparten::KruskalTensorInitializer<double, sparten::type::SubIdx> kruskalInit;
